...
def brazo_sup_izq():
    glPushMatrix()
    glScalef(BRAZO_INF_ALTO, BRAZO_INF_RADIO, BRAZO_SUP_RADIO)
    glRotatef(-90, 0, 1, 0)
    glutSolidSphere(1, SLICES, STACKS)
    glPopMatrix()
...
def brazo_inf_izq():
    glPushMatrix()
    glScalef(BRAZO_INF_ALTO, BRAZO_INF_RADIO, BRAZO_INF_RADIO)
    glRotatef(-90, 0, 1, 0)
    glutSolidSphere(1, SLICES, STACKS)
    glPopMatrix()
...
def brazo_sup_der():
    glPushMatrix()
    glScalef(BRAZO_SUP_ALTO, BRAZO_SUP_RADIO, BRAZO_SUP_RADIO)
    glRotatef(-90, 0, 1, 0)
    glutSolidSphere(1, SLICES, STACKS)
    glPopMatrix()
...
def dibujar_pupete(x, y, z,  # Posicion absoluta
    ...
    if True:  # Brazo derecho
        glTranslatef(-(TORSO_RADIO + BRAZO_SUP_ALTO / 2), 0, 0)
        glRotatef(bsd, 0, 0, 1)
        brazo_sup_der()
        glTranslatef(-(BRAZO_SUP_ALTO + BRAZO_INF_ALTO), 0, 0)
        glRotatef(bid, 0, 0, 1)
        brazo_inf_der()
    glPopMatrix()
    glPushMatrix()
